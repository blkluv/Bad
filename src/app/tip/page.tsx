...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const ogImageUrl = `https://baddie.stream/api/tip-card?user=${encodeURIComponent(user)}&amount=${amount}&tips=${tips}`;
...
export default async function TipPage({ searchParams }: Props) {
  ...
  return (
    <html>
      <head>
        <meta httpEquiv="refresh" content="2;url=https://baddie.stream" />
      </head>
      <body style={{ margin: 0, backgroundColor: "#fff", display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
        <div style={{ textAlign: "center", padding: "2rem" }}>
          ...
          <a href="https://baddie.stream" style={{ color: "#2563EB", fontSize: "14px" }}>Go to Shyft →</a>
        </div>
      </body>
    </html>
  );
